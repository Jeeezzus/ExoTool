  ...
  Surface sdRoundBox( vec3 p, vec3 b, float r , vec3 col)
{
  ...
  vec3 colorCO = vec3(0.12,0.1,0.1);

  Surface sdScene(vec3 p) {
    Surface co = sdCapsule(p, center1, P11, 0.2*1.3, colorCO);
    co = minWithColor(co, sdCapsule(p, P11, P12, 0.18*1.3, colorCO));
    co = minWithColor(co, sdCapsule(p, P12, P13, 0.16*1.3, colorCO));
    
    co = minWithColor(co, sdCapsule(p, center2, P21, 0.19*1.3, colorCO));
    co = minWithColor(co, sdCapsule(p, P21, P22, 0.18*1.3, colorCO));
    co = minWithColor(co, sdCapsule(p, P22, P23, 0.175*1.3, colorCO));
    
    co = minWithColor(co, sdCapsule(p, center3, P31, 0.18*1.3, colorCO));
    co = minWithColor(co, sdCapsule(p, P31, P32, 0.17*1.3, colorCO));
    co = minWithColor(co, sdCapsule(p, P32, P33, 0.16*1.3, colorCO));
    
    co = minWithColor(co, sdCapsule(p, center4, P41, 0.16*1.3, colorCO));
    co = minWithColor(co, sdCapsule(p, P41, P42, 0.15*1.3, colorCO));
    co = minWithColor(co, sdCapsule(p, P42, P43, 0.15*1.3, colorCO));

    co = minWithColor(co, sdCapsule(p, center0, P01, 0.21*1.3, colorCO));
    co = minWithColor(co, sdCapsule(p, P01, P02, 0.2*1.3, colorCO));
    co = minWithColor(co, sdCapsule(p, P02, P03, 0.21*1.3, colorCO));

    co = minWithColor(co, sdRoundBox(p + vec3(-0.1,2.1,-0.2), vec3(0.000000001,1.,0.75), 0.3, colorCO));
    return co;
  }
